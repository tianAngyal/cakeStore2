...
const renderModal = () => {
  ...
  const mappedList = store.addedToCard
    .map((props) => {
      const markupList = `
        <li class="item_desktop">
          <img src="${props.images.thumbnail}"/>
          <div class="item_separator">
          <div class="separator2">
          <h2 class="title">${props.name}</h2>
              <div class="amount-info"> 
                <span class="amount price">${props.quantity}x</span>
                <span class="price-per-item item-title">${
                  euro(props.price) + euro(props.price).s.symbol
                }</span>
              </div>
              </div>
              <span class="price-total">${
                euro(props.sum) + euro(props.sum).s.symbol
              }</span>
          </div>
        </li>`;
      return markupList;
  ...
  const markupModal = `
        <div class="confirmation">
          ...
          <h1 class="confirmation_title">Order Confirmed</h1>
          <p class="confirmation_text">We hope you enjoy your food!</p>
          <form class="confirmation_items">
            <ul class="list">
              ${mappedList}
              </ul>
              <div class="item_separator extension">
              ...
              <button id="send_confirmation" class="confirm" type="submit">Start New Order</button>
        </div>`;
  return modal.insertAdjacentHTML("beforeend", markupModal);
};
